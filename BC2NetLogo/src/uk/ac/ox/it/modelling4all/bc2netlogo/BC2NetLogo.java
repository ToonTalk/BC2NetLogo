...
//	int includesIndex = modelString.indexOf("__includes");
//	if (includesIndex >= 0) {
//	    // remove the includes statement line
//	    int endIndex = modelString.indexOf("]", includesIndex);
//	    modelString = modelString.substring(0, includesIndex) + modelString.substring(endIndex+2);
//	}
//	String codeWithoutGlobalsAndGlobals[] = removeGlobals(auxiliaryFile);
//	String modelCode = codePartOf(modelString);
//	modelCode = addGlobals(modelCode, codeWithoutGlobalsAndGlobals[1]);
	// move images and data to a more reasonable default location
//	modelCode = modelCode.replace("../netlogo/", "BC2NetLogoData/");
	String completeModelString = modelString; // modelCode + "\n" + codeWithoutGlobalsAndGlobals[0] + noncodePartOf(modelString);
	final String codeExperimentsAndRestOfModel[] = new String[3];
	Runnable checkProceduresRunnable = new Runnable() {

	    public void run() {
		try {
		    ModelSaver modelSaver = new ModelSaver(App.app());
		    proceduresExperimentsAndRestOfModel(modelSaver.save(), codeExperimentsAndRestOfModel);
		} catch (Exception ex) {
...
//    }
    ...
    /**
     * @param modelWithoutProcedures
     * @param experimentsAndRestOfModel
     * 
     * [0] is the content of the procedures tab
     * if an experiment is contained in modelWithoutProcedures extracts it and puts in [1]
     * [2] is the remainder of modelWithoutProcedures
     */
...
//    private static String addGlobals(String code, String globals) {
//	// the space after [ is needed below to avoid confusion with
//	// initialise-globals [...
//	// but since code is generated by the Behaviour Composer there is no problem
//	int indexOfGlobals = code.indexOf("globals [ ");
//	if (indexOfGlobals < 0) {
//	    return "globals [ " + globals + "]\n" + code;
//	} else {
//	    int indexOfGlobalsEnd = code.indexOf("]", indexOfGlobals);
//	    return code.substring(0, indexOfGlobalsEnd) + "\n         " + globals + " " + code.substring(indexOfGlobalsEnd);
//	}
//    }
//
//    private static String[] removeGlobals(String code) {
//	// returns code without globals declaration and the global variables
...
//	} else {
//	    int indexOfGlobalsEnd = code.indexOf("]", indexOfGlobals);
//	    if (indexOfGlobalsEnd >= 0) {
//		result[0] = code.substring(0, indexOfGlobals) + code.substring(indexOfGlobalsEnd+2);
//		result[1] = code.substring(indexOfGlobals+9, indexOfGlobalsEnd);
//	    } else {
//		result[0] = code;
//		result[1] = "";
//	    }
...
//    }
//
//    private static String codePartOf(String modelString) {
//	int codeEnd = modelString.indexOf("@#$#@#$#@");
//	if (codeEnd >= 0) {
//	    return modelString.substring(0, codeEnd);
//	}
//	return null;
//    }
//    
//    private static String noncodePartOf(String modelString) {
//	int codeEnd = modelString.indexOf("@#$#@#$#@");
//	if (codeEnd >= 0) {
//	    return modelString.substring(codeEnd);
//	}
//	return null;
//    }
